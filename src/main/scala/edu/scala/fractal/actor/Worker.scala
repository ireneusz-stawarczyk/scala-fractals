...
import akka.actor._
import edu.scala.fractal.mandelbrot.{MandelbrotAlgorithm, JuliaAlgorithm}
...
class Worker(val width : Int, val height : Int) extends Actor {
  private[this] val julia = JuliaAlgorithm(width, height)
  private[this] val mandelbrot = MandelbrotAlgorithm(width, height)

  def receive = {
    case work : Work =>
      sender ! WorkResult(work.x, work.y, calculate(work))
  }

  def calculate(work : Work) : Int = {
  	if (work.algorithm == mandelbrot.algorithm) {
      mandelbrot.calculateRgb(work.x, work.y, work.zoom, work.moveX, work.moveY)
    } else {
      julia.calculateRgb(work.x, work.y, work.zoom, work.moveX, work.moveY)
    }
  }
}
